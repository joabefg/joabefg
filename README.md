# Joab Martins
### Aluno SESI/SENAI 3° Ano | Analista de Dados | Programador Pyhon

### 🔨 Toolbox Técnico
- **IDE**: Visual Studio Code
- **Linguagens**: Python, Ecosistema JavaScript
- **Versionamento**: Git, Github
- **Database**: MySQL
- **Plataformas**: Desktop, Mobile (Android e IOS), Web 

### 🎓 Cursos Técnicos
- **Desenvolvimento de Sistemas**: SENAI
- **Python Básico**: Cisco, 120h

### 🏆 Projetos em Destaque
- [Pokedex](github.com/joabefg/pokedex): Projeto React Native Expo

### ✉️ Contato
[![Gmail] (https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](joab.martins@docente.senai.br)